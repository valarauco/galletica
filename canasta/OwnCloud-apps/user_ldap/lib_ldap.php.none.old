...
class OC_LDAP {
	...
	static protected $ldapTTL;
	static protected $ldapLastUpdate;
	...
	//valarauco
	static public function ocnameUserList() {
		return self::ocnameList(true);
	}
	static public function ocnameGroupList() {
		return self::ocnameList(false);
	}
	static public function ocnameList($isUser = true) {
		if($isUser) {
			$knownObjects = self::mappedOCUsers();
		} else {
			$knownObjects = self::mappedOCGroups();
		}
		$ownCloudNames = array();
		foreach($knownObjects as $object) {
			$ownCloudNames[] = $object['owncloud_name'];
		}
	...
	//valarauco
	static private function mappedOCGroups() {
		return self::mappedComponents(false,'oc');
	}

	static private function mappedOCUsers() {
		return self::mappedComponents(true,'oc');
	}

	//valarauco EDIT
	static private function mappedComponents($isUsers, $column='all') {
	...
	static private function search($filter, $base, $attr = null) {
		...
		// valarauco limit search to 50
		//$sr = @ldap_search(self::getConnectionResource(), $base, $filter, $attr,0,50);
	...
	static private function readConfiguration($force = false) {
		if(!self::$configured || $force) {
			...
			// valarauco
			self::$ldapTTL       				 = OCP\Config::getAppValue('user_ldap', 'ldap_ttl', '1440');
			self::$ldapLastUpdate				 = OCP\Config::getAppValue('user_ldap', 'ldap_lastupdate');
