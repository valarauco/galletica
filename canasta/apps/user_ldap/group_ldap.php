...
// 	//group specific settings
	...
	public function getUserGroups($uid) {
		$manuuid = $uid;
		error_log('[LDAP-G] getUserGroups: '.$manuuid);
		if(!$this->configured) {
			return array();
		}
		...
		$this->_user_groups[$manuuid] = array_unique(OC_LDAP::ownCloudGroupNames($groups), SORT_LOCALE_STRING);
		error_log('[LDAP-G] getUserGroups: $uid '.$manuuid.' OC_groups: '.implode(' ',$this->_user_groups[$manuuid]));
		error_log('[LDAP-G] getUserGroups: FIN!!');
		//return $this->_user_groups[$uid];
		return $this->_user_groups[$manuuid];
	}
	...
	public function usersInGroup($gid) {
		...
		error_log('[LDAP-G] usersInGroup: $gid '.$gid.' OC_ucers: '.implode(' ',$this->_group_users[$gid]));
